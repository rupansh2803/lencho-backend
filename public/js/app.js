...
function productCardHTML(p) {
  ...
  return `
  <div class="product-card reveal lencho-stable-card" role="button" tabindex="0" onclick="navigate('${safeDetailPath}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();navigate('${safeDetailPath}')}" style="border-radius:16px !important;overflow:hidden;box-shadow:0 4px 16px rgba(0,0,0,.08) !important;transition:box-shadow .25s ease,border-color .25s ease !important;background:#fff;border:1px solid rgba(201,106,138,.08);cursor:pointer;">
